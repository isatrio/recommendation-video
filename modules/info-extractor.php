...
//	    $docs = preg_split('/\r\n|\r|\n/',$text);
        ...
        foreach ($docs as $doc) {
            ...
            // Stemming process to change word to be a base word
            ...
            $cleaned = $remover->remove($doc);

            array_push($result, $cleaned);
        }
    ...
    private function mergeDocument($docs) {
        foreach ($docs as $doc) {
    ...
    {
        $summary = [];
        $tfidfResult = $this->tfidf();
        $threshold = 1 * $this->average($tfidfResult);

        foreach ($tfidfResult as $word => $score) {
