...
//        echo '<pre>';
        for ($i = 0; $i <= sizeof($this->tset); $i++) {
            if ($this->tset[$i] !== null) {
                $tfidf = $ff->getFeatureArray("", $this->tset[$i]);
                $mergedArray += $tfidf;
            }
        }

        $threshold = 1 * $this->average($mergedArray);
//        print_r($mergedArray);

        foreach ($mergedArray as $word => $score) {
            if ($score < $threshold && $word !== '') $summary[] = $word;
        }
